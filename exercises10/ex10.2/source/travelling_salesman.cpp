...
CitiesLocation::CitiesLocation(double x[], double y[], unsigned int n_cities) :
    positions(n_cities),
    l1(n_cities, std::vector<double>(n_cities)),
    l2(n_cities, std::vector<double>(n_cities))
{
...
std::vector<double> CitiesLocation::getX(){
...
std::vector<double> CitiesLocation::getY(){
...
AnnealingTSP::AnnealingTSP(CitiesLocation citiesloc,
                           std::string _norm,
                           unsigned int thread_id) :
    cities(citiesloc),
    norm(_norm),
    path(citiesloc.getSize()),
    rand("Primes", "seed.in", thread_id)
{
    if(norm != "L1" and norm != "L2"){
        std::cout << "'"<<norm <<"' is not a valid norm.\n";
                                         ...
                                         bool return_costs)
{
    ...
    std::cout << "accepted " << steps-acce << std::endl;
    return costs;
}
