...
//	return k_elastic*val*val/2.0;
    return std::pow(x,4)-5.*std::pow(x,2)/2.;
}
...
{
    return exp(-pow(v-mu,2)/(2.*pow(sigma, 2)))+
            exp(-pow(v+mu, 2)/(2.*pow(sigma,2)));
}

double variationalWaveFunction_second(double v)
{
    double s2 =sigma*sigma;
    double exp1=exp(-pow(v-mu,2)/(2*s2));
    double exp2 = exp(-pow(v+mu,2)/(2*s2));
    return (pow((v-mu)/s2,2)-1/s2)*exp1+(pow((v+mu)/s2,2)-1/s2)*exp2;
}
